...
function getLikeCount(contentId) {
	const counts = loadMapStorage(STORAGE_KEYS.likeCounts);
	return counts[contentId] || 0;
}

function incrementLikeCount(contentId) {
	const counts = loadMapStorage(STORAGE_KEYS.likeCounts);
	counts[contentId] = (counts[contentId] || 0) + 1;
	saveMapStorage(STORAGE_KEYS.likeCounts, counts);
	return counts[contentId];
}
...
function ensureNewUsersStartAtRegister() {
	...
	// Allow auth pages and saved (which handles unauthenticated state gracefully)
	const openPages = ["register.html", "login.html", "saved.html"];

	if (!hasUsers && !openPages.includes(page)) {
...
function renderFeed() {
	...
	feedList.innerHTML = posts
		.map((post) => {
			...
			const creatorAvatar = getCreatorAvatar(post.creatorKey);
			const hashtagHtml = (post.hashtags || []).map(tagLinkHtml).join(" ");
			const soundInfo = SOUND_DIRECTORY[post.sound] || {};
			const soundName = soundInfo.name || getSoundLabel(post.sound);
			const likeCount = getLikeCount(post.id);
			const likeCountDisplay = likeCount > 0 ? likeCount.toLocaleString() : "0";
			const mediaHtml = hasMedia
				? `<video class="post-video" style="filter:${videoFilterCss};" playsinline loop preload="metadata" src="${post.mediaDataUrl}"></video>`
				: `<div class="video-placeholder${post.style === "alt" ? " alt" : ""}"><div class="placeholder-inner"><img class="placeholder-logo" src="assets/jeoq-logo.svg" alt="JEOQ" /><p class="placeholder-text">"${post.title}"</p></div></div>`;
			return `<article class="video-post" data-content-id="${post.id}" data-content-title="${post.title}">
				${mediaHtml}
				<div class="video-gradient"></div>
				<div class="video-overlay">
					<div class="video-overlay-left">
						<div class="creator-row">
							<a class="creator-link" href="creator.html?creator=${post.creatorKey}">
								<img class="creator-avatar-sm" src="${creatorAvatar}" alt="${post.creator}" />
							</a>
							<a class="creator-link" href="creator.html?creator=${post.creatorKey}">${post.creator}</a>
						...
						<div class="sound-row">
							<a class="sound-disc-link" href="sound.html?sound=${post.sound}" aria-label="Sound: ${soundName}">
								<img class="sound-disc-spin" src="${creatorAvatar}" alt="Sound disc" />
							</a>
							<span class="sound-name-marquee">${soundName}</span>
						</div>
					</div>
					<div class="video-side-actions">
						<button class="action-btn icon-action side-action" data-action="like" aria-label="Like">
							<span class="side-action-icon">\u2764\uFE0F</span>
							<span class="side-action-count like-count" id="lc-${post.id}">${likeCountDisplay}</span>
						</button>
						<button class="action-btn icon-action side-action" data-action="favorite" aria-label="Favorite">
							<span class="side-action-icon">\u2B50</span>
							<span class="side-action-count">Save</span>
						</button>
						<a class="sound-link side-action" href="sound.html?sound=${post.sound}" aria-label="Open sound">
							<span class="side-action-icon">\u{1F3B5}</span>
							<span class="side-action-count">Sound</span>
						</a>
					</div>
				</div>
			</article>`;
...
function getCreatorAvatar(creatorKey) {
	if (creatorKey && CREATOR_DIRECTORY[creatorKey]) {
		return CREATOR_DIRECTORY[creatorKey].avatar || "assets/jeoq-logo.svg";
	}
	const users = loadUsers();
	if (creatorKey && users[creatorKey]) {
		return users[creatorKey].avatar || "assets/jeoq-logo.svg";
	}
	return "assets/jeoq-logo.svg";
}
function updateNavAvatar() {
	const avatarNodes = document.querySelectorAll("#nav-avatar, .bnav-avatar");
	...
	document.querySelectorAll(".auth-form[data-auth]").forEach((form) => {
		form.addEventListener("submit", (event) => {
			...
			if (mode === "register") {
				...
				showToast("Account created! Welcome to JEOQ.", "success");
				form.reset();
...
function bindContentActions() {
	...
	document.addEventListener("click", (event) => {
		...
		if (post && (actionKey === "like" || actionKey === "favorite")) {
			...
			const itemIndex = existing.findIndex((item) => item.id === contentId);
			...
			if (itemIndex === -1) {
				...
				if (actionKey === "like") {
					const newCount = incrementLikeCount(contentId);
					const countEl = document.getElementById(`lc-${contentId}`);
					if (countEl) countEl.textContent = newCount.toLocaleString();
				}
				button.classList.add("is-active");
				const action = actionKey.replace(/-/g, " ");
				setStatus(status, `${action} added.`);
			} else {
				...
				if (actionKey === "like") {
					...
					if (counts[contentId] > 0) {
						counts[contentId] = counts[contentId] - 1;
						saveMapStorage(STORAGE_KEYS.likeCounts, counts);
					}
					const countEl = document.getElementById(`lc-${contentId}`);
					if (countEl) {
						const newCount = counts[contentId] || 0;
						countEl.textContent = newCount > 0 ? newCount.toLocaleString() : "";
					}
				}
				button.classList.remove("is-active");
				const action = actionKey.replace(/-/g, " ");
				setStatus(status, `${action} removed.`);
			}
		}
	});
}
...
function markSavedButtons() {
	...
	const favItems = getSavedItemsForUser(STORAGE_KEYS.favorited, current.key);
	...
	document.querySelectorAll(".video-post").forEach((post) => {
		...
		const favBtn = post.querySelector('[data-action="favorite"]');
		if (likeBtn) likeBtn.classList.toggle("is-active", likedIds.has(contentId));
		if (favBtn) favBtn.classList.toggle("is-active", favIds.has(contentId));
	});
}
...
function logoutUser() {
	...
	setTimeout(() => redirect("register.html"), 400);
}
...
function bindCharCounter() {
	...
	descField.addEventListener("input", () => {
		...
		counter.classList.toggle("char-over", len > max);
	});
}
...
function renderProfilePage() {
	...
	function showProfile(userKey, userData) {
		...
		profilePassword.textContent = userData.password ? "•".repeat(Math.min(userData.password.length, 12)) : "-";
...
function bindDeleteAccount() {
	...
	deleteForm.addEventListener("submit", (event) => {
		...
		const current = getCurrentUser();
		if (!current) {
			setStatus(profileStatus, "No active session. Please log in first.");
			return;
		}

		const username = normalizeUsername(deleteForm["confirm-username"].value);
		const password = String(deleteForm["confirm-password"].value || "").trim();
		const users = loadUsers();

		if (username !== current.key) {
			setStatus(profileStatus, "Username does not match. Account deletion cancelled.");
			return;
		}

		if (users[current.key].password !== password) {
			setStatus(profileStatus, "Password is incorrect. Account deletion cancelled.");
			return;
		}
		...
		const liked = loadMapStorage(STORAGE_KEYS.liked);
		delete liked[current.key];
		saveMapStorage(STORAGE_KEYS.liked, liked);

		const favorited = loadMapStorage(STORAGE_KEYS.favorited);
		delete favorited[current.key];
		saveMapStorage(STORAGE_KEYS.favorited, favorited);

		setStatus(profileStatus, "Account deleted successfully. You will be redirected.");
		showToast("Account deleted. Redirecting to registration page...", "success");
		
		modal.classList.add("is-hidden");
		modal.setAttribute("aria-hidden", "true");
		deleteForm.reset();

		setTimeout(() => redirect("register.html"), 1500);
	});
}
...
function bindTouchSwipe() {
	...
	let startY = 0;
	feedColumn.addEventListener("touchstart", (e) => {
		startY = e.touches[0].clientY;
	}, { passive: true });
	feedColumn.addEventListener("touchend", (e) => {
		const delta = startY - e.changedTouches[0].clientY;
		if (Math.abs(delta) < 50) {
			return;
		}
		const posts = feedColumn.querySelectorAll(".video-post");
		let currentIndex = 0;
		...
		if (delta > 0 && currentIndex < posts.length - 1) {
